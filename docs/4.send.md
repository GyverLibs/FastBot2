...
### Второй способ
Подходит для json пакетов, собранных отдельно. Работает чуть медленнее, чем beginPacket-sendPacket, но использует столько же оперативки (не больше, не дублирует пакет в памяти). Отправлять можно как json объект вида `{"key":"value"...}`, так и список значений без фигурных скобок (библиотека добавит их сама): `"key":"value"...`. Например:
```cpp
// свой пакет
bot.sendCommand(fb::cmd::sendMessage, "{\"chat_id\":1234567864,\"text\":\"hello text\"}");

// сборка в gson::string
gson::string g;
g[fb::api::chat_id] = 1234567864;
g[fb::api::text] = "hello text";
bot.sendCommand(fb::cmd::sendMessage, g);
```